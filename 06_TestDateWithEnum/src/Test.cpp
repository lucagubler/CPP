...
void testPrintADate() {
	...
	day.print(os);
	ASSERT_EQUAL("20. Aug 2012", os.str());
}
...
void testDateCtorThrowsIfInvalid() {
	...
	ASSERT_THROWS((Date { 0, jan, 0 }), std::out_of_range);
	ASSERT_THROWS((Date { 1900, feb, 29 }), std::out_of_range);
	ASSERT_THROWS((Date { 2011, apr, 31 }), std::out_of_range);
}
...
void testNextDayEndOfFeburaryNonLeapYear() {
	Date eofeb {feb, 28 , 2013};
	eofeb.nextDay();
	ASSERT(datesAreEqual(eofeb, Date { 2013, mar, 1 }));
}
void testNextDayNotEndOfFeburaryInLeapYear() {
	Date aday {  feb, 28, 2016 };
	aday.nextDay();
	ASSERT(datesAreEqual(aday, Date { 2016, feb, 29 }));
}

void testNextDayEndOfYear() {
	Date silvester { 2015, december, 31 };
	silvester.nextDay();
	ASSERT(datesAreEqual(silvester, Date { 2016, jan, 1 }));
}
...
void testDateReadValidDates() {
	...
	Date aday { 2000, jan, 1 };
	Date expect { 2012, aug, 17 };
	aday.read(is);
	ASSERT(!is.fail());
...
void testDateReadShortYear() {
	...
	aday.read(is);
	ASSERT(!is.fail());
	ASSERT_EQUAL((Date { 2012, aug, 17 }), aday);
	aday.read(is);
	ASSERT(!is.fail());
	ASSERT_EQUAL((Date { 2013, sep, 1 }), aday);
}
...
void testDateReadYYYYMMDD() {
	...
	aday.read(is);
	ASSERT(!is.fail());
	ASSERT_EQUAL((Date { 2012, aug, 20 }), aday);
}
...
// not possible:
//static_assert(std::numeric_limits<date::Month>::max(),"numeric limits for enums?");
void testDefinitionsOnly() {
	Month nice { may };
	Month alsoMay = Month(5);
	Month pryToBeMay = static_cast<Month>(feb + 3);
	ASSERT_EQUAL(nice, alsoMay);
	ASSERT_EQUAL(nice, pryToBeMay);
	auto i = nice + alsoMay + pryToBeMay;
	static_assert(std::is_same<decltype(i),int>::value,"adding enums makes an int");
}

enum class color {
	red, yellow, green, max_color = green
};
...
void testMakeDateYYYYMMDD() {
	...
	ASSERT(!is.fail());
	ASSERT_EQUAL((Date { 2012, aug, 20 }), aday);
	ASSERT_EQUAL((Date { 9999, dec, 31 }), make_date(is));
}

void testInputOperatorDateButLessThan() {
	std::istringstream is { "2012-08-20" };
	Date aday { 2001, jan, 26 };
	//is > aday; // compiles without explicit

}

void testDefaultDate() {
	ASSERT_EQUAL((Date { 9999, dec, 31 }), Date { });
}
void testWeekdayOutput(){
	std::ostringstream out{};
	out << Date::Monday << Date::Tuesday << Date::Wednesday
		<< Date::Thursday << Date::Friday << Date::Saturday
		<< Date::Sunday;
	ASSERT_EQUAL("MondayTuesdayWednesdayThursdayFridaySaturdaySunday", out.str());
}

void testgetDayOfWeekForDay(){
	Date const birthday{december,24,1964};
	ASSERT_EQUAL(Date::Thursday,birthday.day_of_week());
	ASSERT_EQUAL(Date::Wednesday,Date(nov,11,2015).day_of_week());
}
...
void runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testWeekdayOutput));
	s.push_back(CUTE(testgetDayOfWeekForDay));
